...
sudo zypper in gcc-c++ gcc-fortran python make
...
#verifica que la instalacion cuente con f77, en caso de no ser asi realiza un enlace simbolico de gfortran
if test -f "/usr/bin/f77"; then
	:
else
...
cd build
if test -f "scalapack_installer.tgz"; then
	:
else
	...
	chmod u+x scalapack_installer.tgz
fi

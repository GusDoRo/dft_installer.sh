...
./setup.py --prefix=./opt-O4 --mpibindir=$dft_instalacion/build/mpich-$dft_mpich_version/install/bin --mpiincdir=$dft_instalacion/build/mpich-$dft_mpich_version/install/include \
		--ccflags="-O4 -funroll-all-loops -ftree-vectorize -fallow-argument-mismatch" --fcflags="-O4 -funroll-all-loops -ftree-vectorize -fallow-argument-mismatch" --noopt="-fallow-argument-mismatch" --downall
...
if test -f $dft_instalacion"/build/scalapack_installer/opt-O4/lib/libscalapack.a"; then
	:
else
	./setup.py --prefix=./opt-O4 --mpibindir=$dft_instalacion/build/mpich-$dft_mpich_version/install/bin --mpiincdir=$dft_instalacion/build/mpich-$dft_mpich_version/install/include \
		--ccflags="-O4 -funroll-all-loops -ftree-vectorize -fallow-argument-mismatch" --fcflags="-O4 -funroll-all-loops -ftree-vectorize -fallow-argument-mismatch" --noopt="-fallow-argument-mismatch" --downall --notesting
	if test $? -ne 0; then
		echo $'\n\n...\n\nHubo un error'
		exit 1
	fi

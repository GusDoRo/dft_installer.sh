...
#cp fftw-3.3.8.tar.gz ./build/
cd build/fftw-$dft_fftw_version
...
export CFLAGS="-O4 -funroll-all-loops -ftree-vectorize"

export FFLAGS="-O4 -funroll-all-loops -ftree-vectorize"

./configure --prefix=$dft_instalacion/build/fftw-$dft_fftw_version/install-O4 #--enable-mpi
if test $? -ne 0; then
	echo $'\n\n...\n\nHubo un error'
	exit 1
fi

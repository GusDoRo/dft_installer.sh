...
then
	...
	sufijoop="-"$sufijoop
	banderas=-O4 -funroll-all-loops -ftree-vectorize
fi
sufijo="-siesta-"$sufijo$sufijoop

#directorio
cd build/$ruta

#creando directorio de trabajo
cp -r Util Util-$optimizacion
cp Obj-$optimizacion/arch.make Obj/arch.make
cd Util-$optimizacion

#Bands
cd $dft_instalacion/build/$ruta/Util-$optimizacion/Bands
make all
gfortran gnubands.f $banderas -o gnubands #no se hace solo
#if test $? -ne 0; then
#	echo $'\n\n...\n\nHubo un error'
#	exit 1
#fi

cp eigfat2plot $dft_instalacion/bin/eigfat2plot$sufijo
cp new.gnubands $dft_instalacion/bin/new.gnubands$sufijo
cp gnubands $dft_instalacion/bin/gnubands$sufijo

#Contrib
cd $dft_instalacion/build/$ruta/Util-$optimizacion/Contrib/APostnikov
make all
if test $? -ne 0; then
	echo $'\n\n...\n\nHubo un error'
	#exit 1
...
cp fmpdos $dft_instalacion/bin/fmpdos$sufijo

#Denchar
cd $dft_instalacion/build/$ruta/Util-$optimizacion/Denchar/Src
make
if test $? -ne 0; then
	echo $'\n\n...\n\nHubo un error'
	#exit 1
...
cp denchar $dft_instalacion/bin/denchar$sufijo

#TBTrans
cd $dft_instalacion/build/$ruta/Util-$optimizacion/TBTrans
make
if test $? -ne 0; then
	echo $'\n\n...\n\nHubo un error'
	#exit 1
...
cp tbtrans $dft_instalacion/bin/tbtrans$sufijo

#COOP
cd $dft_instalacion/build/$ruta/Util-$optimizacion/COOP
make all
if test $? -ne 0; then
	echo $'\n\n...\n\nHubo un error'
	#exit 1
...
cp mprop $dft_instalacion/bin/mprop$sufijo

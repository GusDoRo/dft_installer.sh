...
#directorio y descompresión de mpich
cp mpich-$dft_mpich_version.tar.gz ./build/
cd build

tar -xzvf mpich-$dft_mpich_version.tar.gz

cd mpich-$dft_mpich_version

mkdir build

#para evitar un error en algunas versiones del complilador
export FFLAGS="-fallow-argument-mismatch"

#configurando mpich con ./install como directorio de instalación
mkdir install
cd build

../configure -prefix=$dft_instalacion/build/mpich-$dft_mpich_version/install 2>&1 | tee c.txt

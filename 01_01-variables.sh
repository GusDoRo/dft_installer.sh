...
#si ya se creo variables no se vuelve a crear
if test -f "tmp/variables.sh"; then
	exit 0
fi
...
echo "dft_instalacion=$( dirname -- "$( readlink -f -- "$0"; )"; )"$'\n' >>variables.sh

tempo=`ls mpich-*.tar.gz`

echo "dft_mpich_version="`expr ${tempo:6:(-7)}`>>variables.sh

tempo=`ls fftw-*.tar.gz`

echo "dft_fftw_version="`expr ${tempo:5:(-7)}`>>variables.sh

echo 'dft_siesta_4_lista="'`ls siesta-4*.tar.gz`'"'>>variables.sh

echo 'dft_siesta_3_lista="'`ls siesta-3*.tgz siesta-trunk*.tgz`'"'>>variables.sh

echo 'dft_siesta_2_lista="'`ls siesta-2*.tgz`'"'>>variables.sh

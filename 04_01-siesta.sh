...
do
	...
	if test -f bin/siesta-$sinpuntos; then
		:
	else
		./siesta-helper.sh `expr ${i:0:(-7)}` .tar.gz $sinpuntos T
...
do
	...
	if test -f bin/siesta-$sinpuntos; then
		...
		./siesta-helper.sh `expr ${i:0:(-4)}` .tgz $sinpuntos
...
do
	...
	if test -f bin/siesta-$sinpuntos; then
		...
		./siesta2-helper.sh `expr ${i:0:(-4)}` .tgz $sinpuntos

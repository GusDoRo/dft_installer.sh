...
if test -f "siesta-rt.zip"; then
	:
else
	echo "SIESTA RT no se encuentra disponible"
	exit 0
fi

#directorio y descompresión de siesta

cp siesta-rt.zip build/
...
tar -xzvf rt-tddft.tar.gz
tar -xzvf siesta-2.0.1.tgz

rm -R siesta-2.0.1/siesta-2.0.1.tar

cp -R siesta-2.0.1 code/

cd code

patch -p0 < tddft.patch
...
~/remplazo.sh ~/arch.make >arch.make

make

chmod a+x siesta

cp siesta ~/bin/siesta-rt-20
...
~/remplazo.sh ~/arch.make-O4 >arch.make

make

chmod a+x siesta

cp siesta ~/bin/siesta-rt-20-O4
